...
#define MAX_I2C_PKTCNT16 116 // 60 // (SMPS_BLK_CNT/2)
#define MAX_I2C_PKTCNT24 76 // 40 // (SMPS_BLK_CNT/3)
...
void TMR1_IRQHandler(void) {
  ...
  if(TMR1_GetITFlag(RB_TMR_IF_CYC_END)) {
	  if(i2c_dev.timer_flg) {
	    ...
	    i2c_dev.i2c_buf_wr++;
	    i2c_dev.i2c_buf_wr &= (I2C_BUF_SIZE-1);
	    i2c_dev.raddr++;
...
void I2CDevTask(void) {
	...
	if(pwr < prd) {
      ...
      if(i2c_dev.i2c_rd_24bit) {
        ...
        i2c_dev.i2c_buf_rd = prd & (I2C_BUF_SIZE -1);
      }
      else
#endif
      {
  ...
  if(app_drv_fifo_write(&app_tx_fifo, (uint8_t *)&i2c_blk, (uint16_t *)&size) == APP_DRV_FIFO_RESULT_SUCCESS) {
    status.all_read_count++;
	  ...
	  if(ble_enable) {
	      ...
	      I2CDevTimerStop();
	      i2c_blk.count = 0;
	      i2c_blk.id |= CMD_ERR_FLG;
	      size = 2;
	      app_drv_fifo_write(&app_tx_fifo, (uint8_t *)&i2c_blk, (uint16_t *)&size);
	  }
	}
}
