...
#if CHIP_ID == ID_CH583
#define APP_TX_BUFFER_LENGTH      (1<<11) // CH583 (RAM 32K) -> fifo: 2048 or 4096 bytes
#else
#define APP_TX_BUFFER_LENGTH      (1<<10) // CH592 (RAM 24K) -> fifo: 1024 bytes
#endif
...
uint16_t
app_drv_fifo_rd(app_drv_fifo_t *fifo, uint8_t *data, uint16_t read_length);

void
app_drv_fifo_rd_del(app_drv_fifo_t *fifo, uint16_t read_length);
